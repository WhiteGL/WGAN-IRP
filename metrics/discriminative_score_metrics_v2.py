...
def discriminative_score_metrics(dataX, dataX_hat):
    ...
    X_hat = tf.compat.v1.placeholder(tf.float32, [None, Max_Seq_Len, data_dim], name="myinput_x_hat")
    ...
    def discriminator(X, T):
        ...
        with tf.compat.v1.variable_scope("discriminator", reuse=tf.compat.v1.AUTO_REUSE) as vs:
            d_cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_units=hidden_dim, activation=tf.nn.tanh, name='cd_cell')

            d_outputs, d_last_states = tf.compat.v1.nn.dynamic_rnn(d_cell, X, dtype=tf.float32, sequence_length=T)

            # Logits
            Y_hat = tf.compat.v1.layers.dense(d_last_states, 1, activation=None)
    ...
    D_loss_real = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=Y_real, labels=tf.ones_like(Y_real)))
    D_loss_fake = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=Y_fake, labels=tf.zeros_like(Y_fake)))
    ...
    Y_pred_real_curr, Y_pred_fake_curr = sess.run([Y_pred_real, Y_pred_fake],
                                                  feed_dict={X: testX, T: testT, X_hat: testX_hat, T_hat: testT})
