...
def predictive_score_metrics(dataX, dataX_hat):
    ...
    X = tf.compat.v1.placeholder(tf.float32, [None, Max_Seq_Len - 1, data_dim - 1], name="myinput_x")
    ...
    D_loss = tf.compat.v1.losses.absolute_difference(Y, Y_pred)
    ...
    for itt in range(iterations):
        ...
        Y_mb = list(
            np.reshape(dataX_hat[i][1:, (data_dim - 1)], [len(dataX_hat[i][1:, (data_dim - 1)]), 1]) for i in train_idx)
